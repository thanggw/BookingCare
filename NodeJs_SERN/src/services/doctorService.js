...
import { sendAttachment } from "./emailService.js";
dotenv.config();
      ...
      if (!inputId) {
        ...
        let data = await db.User.findOne({
          ...
          include: [
            ...
            {
              model: db.Allcode,
              as: "genderData", //tên đinh danh khai báo trong file model user.js , phải có để sequelize nó biết tên của object chứa các thông tin cần trả về
              attributes: ["valueEn", "valueVi"],
            },
            {
              model: db.Allcode,
          ...
          raw: false,
          nest: true, // nếu không có thì thông tin sẽ trả về dạng Markdown.description mà sẽ không hiển thị theo tên
        });
        if (data && data.image) {
